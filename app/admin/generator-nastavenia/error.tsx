...
/**
 * Custom error boundary pre /admin/generator-nastavenia — zobrazuje reálny
 * text chyby (namiesto generic „Application error") aby sa dala rýchlo
 * diagnostikovať bez otvárania DevTools.
 */
export default function GeneratorNastaveniaError({
  error,
  reset,
}: {
  ...
  React.useEffect(() => {
    console.error("[/admin/generator-nastavenia] runtime error:", error);
  }, [error]);
  ...
  return (
    <div className="max-w-2xl mx-auto mt-6 rounded-2xl border-2 border-rose-300 dark:border-rose-800 bg-rose-50 dark:bg-rose-950/40 p-5 space-y-3">
      <div className="flex items-center gap-2 text-rose-900 dark:text-rose-100 font-black text-lg">
        ...
        Chyba v /admin/generator-nastavenia
      </div>
